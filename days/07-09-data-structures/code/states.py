...
def get_every_nth_state(states=states, n=10):
    ...
    while cnt <= len(states):
        try:
            lst.append(states[cnt])
            cnt += n
...
def combine_state_names_and_abbreviations(
    ...
    lst = []
    cnt = 0
    for abrv in us_state_abbrev.values():
    ...
    cnt = 0
    copyStates = list(states[-1::-1])

    copyStates.sort(reverse=True)
    for state in copyStates:
